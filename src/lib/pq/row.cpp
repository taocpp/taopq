...
namespace tao::pq
{
   ...
   {
      if( column >= columns_ ) {
         throw std::out_of_range( internal::printf( "column %zu out of range (0-%zu)", column, columns_ - 1 ) );
      }
   }
   ...
   row row::slice( const std::size_t offset, const std::size_t in_columns ) const
   {
      ...
      if( offset + in_columns > columns_ ) {
         throw std::out_of_range( internal::printf( "slice (%zu-%zu) out of range (0-%zu)", offset, offset + in_columns - 1, columns_ - 1 ) );
      }
